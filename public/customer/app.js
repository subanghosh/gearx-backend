...
if (window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.App) {
    window.Capacitor.Plugins.App.addListener('backButton', () => {
        ...
        const modals = [
            'hire-driver-searching-preview', 'hire-driver-accepted-preview',
        ...
        for (const id of modals) {
            ...
            if (el) {
                ...
                if (isVisible) {
                    if (id === 'hire-driver-searching-preview') {
                        if (typeof window.cancelHireDriverSearch === 'function') window.cancelHireDriverSearch();
                        else if (typeof window.closeHireDriverSearchingPreview === 'function') window.closeHireDriverSearchingPreview();
                        else el.style.display = 'none';
                    } else if (id === 'hire-driver-accepted-preview') {
                        if (typeof window.closeHireDriverAcceptedPreview === 'function') window.closeHireDriverAcceptedPreview();
                        else el.style.display = 'none';
                    } else if (id === 'finding-marshal-screen') {
                        const backBtn = document.getElementById('btn-back-marshal-search');
                        if (backBtn && backBtn.style.display !== 'none') {
        ...
        directionsService.route({
            ...
            if (status === 'OK') {
                ...
                if (window.previewHireDriverState.bookingMode === 'km') {
                    window.calculateHireDriverFloorAndOffer(parseFloat(distKm));
                    window.updatePreviewOfferDisplay();
                }
...
window.openHireDriverPreviewScreen = function() {
    ...
    // Sync current vehicle if available
    const vName = document.getElementById('preview-vehicle-name');
    const vPlate = document.getElementById('preview-vehicle-plate');
    const vImg = document.getElementById('preview-vehicle-img');
    const vSpecs = document.getElementById('preview-vehicle-specs');
    
    if (window.selectedVehicle) {
        if (vName) vName.textContent = `${window.selectedVehicle.make || ''} ${window.selectedVehicle.model || 'Vehicle'}`.trim();
        if (vPlate) vPlate.textContent = window.selectedVehicle.plate || 'WB01AB1234';
        if (vSpecs) vSpecs.textContent = `${window.selectedVehicle.transmission || 'Manual'} • ${window.selectedVehicle.fuelType || 'Petrol'}`;
        if (vImg) vImg.src = window.selectedVehicle.photo || (typeof getVehicleIllustration === 'function' ? getVehicleIllustration(window.selectedVehicle.type || 'Car') : 'images/sedan.png');
    }
    
    // Render all pill components
...
window.startHireDriverLiveSearch = async function(customRequestId = null) {
    ...
    window.customerBiddingState.isAccepting = false;
    window.customerBiddingState.isCancelling = false;
    ...
    try {
        ...
        if (!bRes.ok) {
            ...
            showToast(errData.error || 'Failed to broadcast offer to drivers. Please try again.', 'error');
            return;
        }

        const bData = await bRes.json();
        window.customerBiddingState.marshalsNotified = bData.marshalsNotified || 0;
        console.log('[BIDDING] Broadcast result:', bData);
    } catch (e) {
        console.warn('[BIDDING] Broadcast error:', e);
        showToast('Network error while broadcasting offer. Please retry.', 'error');
        return;
    }
...
window.pollCustomerBidOffers = async function() {
    ...
    try {
        ...
        // 2. Render active counter-offers or helpful empty state in searching sheet
        ...
        const currentAmount = window.previewHireDriverState ? window.previewHireDriverState.offerAmount : 550;
                    ...
                    <div style="background: rgba(0, 0, 0, 0.5); border: 1.5px solid rgba(250, 204, 21, 0.35); border-radius: 14px; padding: 10px 12px; display: flex; align-items: center; justify-content: space-between; gap: 8px;">
                        <div style="display: flex; align-items: center; gap: 10px;">
                            ...
                            <div>
                                ...
                                <div style="font-size: 0.68rem; color: #facc15; font-weight: 700; display: flex; align-items: center; gap: 2px;">
                                    ...
                                    ${o.rating} • ${o.distanceKm} km • ~${o.etaMinutes}m
                                </div>
                            </div>
                        </div>
                        <div style="display: flex; align-items: center; gap: 8px;">
                            ...
                            <button type="button" id="btn-accept-counter-${o.id}" onclick="customerAcceptDriverCounter('${o.id}')" style="background: #22c55e; border: none; color: #000; font-size: 0.75rem; font-weight: 900; padding: 8px 14px; border-radius: 8px; cursor: pointer; text-transform: uppercase; box-shadow: 0 2px 10px rgba(34, 197, 94, 0.3);">
                                ACCEPT
                            </button>
                        </div>
                    </div>
                `).join('');
            } else if (window.customerBiddingState.marshalsNotified === 0) {
                offersContainer.style.display = 'flex';
                offersContainer.innerHTML = `
                    <div style="background: rgba(250, 204, 21, 0.08); border: 1.5px dashed rgba(250, 204, 21, 0.35); border-radius: 12px; padding: 10px 14px; text-align: center; color: #facc15; font-size: 0.76rem; font-weight: 700;">
                        No drivers nearby right now.
                        <div style="font-size: 0.66rem; color: #a1a1aa; font-weight: 600; margin-top: 2px;">Tap + to raise offer or wait while radius expands.</div>
                    </div>
                `;
            } else if (offers.length > 0 && offers.every(o => o.status === 'expired' || o.status === 'rejected')) {
                offersContainer.style.display = 'flex';
                offersContainer.innerHTML = `
                    <div style="background: rgba(255, 255, 255, 0.05); border: 1.5px dashed rgba(255, 255, 255, 0.2); border-radius: 12px; padding: 10px 14px; text-align: center; color: #fff; font-size: 0.76rem; font-weight: 700;">
                        No drivers accepted ₹${currentAmount} yet.
                        <div style="font-size: 0.66rem; color: #facc15; font-weight: 600; margin-top: 2px;">Tap + to raise your offer and re-broadcast.</div>
                    </div>
                `;
            } else {
        ...
        if (window.customerBiddingState.autoAccept && driverCounters.length > 0 && !window.customerBiddingState.isAccepting) {
...
window.customerAcceptDriverCounter = async function(offerId) {
    if (window.customerBiddingState.isAccepting) return;
    window.customerBiddingState.isAccepting = true;
    ...
    const btn = document.getElementById(`btn-accept-counter-${offerId}`);
    if (btn) {
        btn.disabled = true;
    ...
    try {
        ...
        if (!res.ok) {
            if (res.status === 409 || data.error === 'TRIP_ALREADY_ASSIGNED') {
                showToast('This driver offer is no longer available.', 'error');
            } else {
                showToast(data.message || data.error || 'Failed to accept driver counter.', 'error');
            }
            return;
        }
...
window.cancelHireDriverSearch = async function() {
    const reqId = window.customerBiddingState.activeRequestId;
    if (window.customerBiddingState.isCancelling) return;
    window.customerBiddingState.isCancelling = true;

    try {
        if (reqId) {
            const res = await fetch(`${API_URL}/service-requests/${reqId}/cancel-bidding`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    ...(localStorage.getItem('token') ? { 'Authorization': `Bearer ${localStorage.getItem('token')}` } : {})
            ...
            // Handle race condition: Driver accepted at the exact moment customer clicked cancel
            if (data && data.alreadyAccepted) {
                ...
                showToast(`${driverName} just accepted your ride! Loading booking details...`, 'success');

                if (data.trip && typeof window.showMarshalEnRoute === 'function') {
                    window.showMarshalEnRoute(data.trip);
                    if (typeof window.startPaymentCountdown === 'function') {
                        window.startPaymentCountdown(data.trip.id);
                    }
        ...
        window.closeHireDriverSearchingPreview();
        showToast('Ride search cancelled.', 'info');
    } catch (e) {
        console.error('Error cancelling search:', e);
        window.closeHireDriverSearchingPreview();
        showToast('Ride search stopped.', 'info');
    } finally {
        window.customerBiddingState.isCancelling = false;
    }
};
...
window.openHireDriverSearchingPreview = function() {
    ...
    if (previewSearchingInterval) clearInterval(previewSearchingInterval);
    previewSearchingInterval = setInterval(() => {
        seconds++;
...
window.stepLiveSearchingOffer = function(delta) {
    ...
    // Debounce backend live broadcast price update by 400ms to batch rapid taps
    if (window.customerBiddingState.stepDebounceTimer) {
        clearTimeout(window.customerBiddingState.stepDebounceTimer);
    }
    
    window.customerBiddingState.stepDebounceTimer = setTimeout(async () => {
        const reqId = window.customerBiddingState.activeRequestId;
        if (reqId) {
            try {
...
window.openHireDriverAcceptedPreview = function() {
    closeHireDriverSearchingPreview();
    const acceptedScreen = document.getElementById('hire-driver-accepted-preview');
    if (!acceptedScreen) return;
    
    const state = window.previewHireDriverState;
    const fareDisp = document.getElementById('accepted-preview-fare-display');
    const plateDisp = document.getElementById('accepted-preview-plate');
    
    if (fareDisp) fareDisp.textContent = `₹${state.offerAmount.toLocaleString('en-IN')}`;
    if (plateDisp && window.selectedVehicle) plateDisp.textContent = window.selectedVehicle.plate || 'WB01AB1234';
    
    acceptedScreen.style.display = 'flex';
};

window.closeHireDriverAcceptedPreview = function() {
    const acceptedScreen = document.getElementById('hire-driver-accepted-preview');
    if (acceptedScreen) acceptedScreen.style.display = 'none';
};

// -------------------------------------------------------------------------
// Trip Type (3-Way: One-Way / Round Trip / Outstation)
// -------------------------------------------------------------------------
// Dynamic calculation of Floor & Suggested Offer using real CRM settings & slabs
window.calculateHireDriverFloorAndOffer = function(customDistKm = null) {
    const s = window.previewHireDriverState;
    const settings = window.redrivoSystemSettings || {};
    const vehicleType = (window.selectedVehicle?.type || 'car').toLowerCase();
    
    const ratePerKm = parseFloat(settings[`${vehicleType}_customer_rate_per_km`]) || (vehicleType === 'car' ? 30 : 8);
    const baseFareFloor = parseFloat(settings[`${vehicleType}_base_fare`]) || (vehicleType === 'car' ? 150 : 50);
    const platformBaseCharge = parseFloat(settings[`${vehicleType}_platform_base_charge`]) || (vehicleType === 'bike' ? 49 : 99);
    const hourlyRate = parseFloat(settings[`${vehicleType}_hourly_rate`]) || (vehicleType === 'car' ? 150 : 80);

    let floor = 350;
    let suggested = 450;

    if (s.tripType === 'outstation') {
        // Outstation rates from CRM slabs (outstation_hourly_slabs & day_incentive_slabs)
        const opt = s.selectedOutstation || '1 Day';
        if (opt === '12 Hrs') { floor = 2160; suggested = 2400; }
        else if (opt === '16 Hrs') { floor = 2560; suggested = 2850; }
        else if (opt === '20 Hrs') { floor = 3000; suggested = 3350; }
        else if (opt === '1 Day') { floor = 2100; suggested = 2350; }
        else if (opt === '2 Days') { floor = 4200; suggested = 4600; }
        else if (opt === '3 Days') { floor = 6300; suggested = 6900; }
        else { floor = 2100; suggested = 2350; }
    } else if (s.bookingMode === 'hourly') {
        const hrs = s.selectedHours || 4;
        floor = Math.round(hrs * hourlyRate);
        suggested = floor + 50;
    } else if (s.bookingMode === 'daywise') {
        const days = s.selectedDays || 1;
        const dailyRate = s.tripType === 'round' ? 2100 : 1500;
        floor = Math.round(days * dailyRate);
        suggested = floor + 150;
    } else {
        // Km / Distance Mode: Real distance based calculation from CRM formulas (Platform Base + max(BaseFare, Dist * Rate))
        const dist = customDistKm !== null ? customDistKm : (s.tripType === 'round' ? 37.0 : 18.5);
        const distanceCharge = Math.max(baseFareFloor, Math.round(dist * ratePerKm));
        floor = Math.round(platformBaseCharge + distanceCharge);
        suggested = Math.round(floor * 1.15 / 10) * 10;
        if (suggested <= floor) suggested = floor + 50;
    }

    s.floorAmount = floor;
    if (!s.userModifiedOffer || s.offerAmount < floor) {
        s.offerAmount = suggested;
    }
...
window.setPreviewTripType = function(type) {
    ...
    if (type === 'oneway') {
        ...
        window.calculateHireDriverFloorAndOffer(18.5);
    } else if (type === 'round') {
        ...
        if (outstationSection) outstationSection.style.display = 'none';
        window.calculateHireDriverFloorAndOffer(37.0);
        ...
        // Clear previously selected drop value when Outstation is selected
        if (dropInput) {
            dropInput.value = '';
            dropInput.placeholder = 'Enter outstation destination (e.g. Digha, Puri)';
        }
        
        window.calculateHireDriverFloorAndOffer(145.0);
    } else {
        if (outstationSection) outstationSection.style.display = 'none';
        if (window.previewHireDriverState.bookingMode === 'km') {
            const modeContainer = document.getElementById('preview-mode-detail-container');
...
window.setPreviewBookingMode = function(mode) {
    ...
    if (mode === 'km') {
        ...
        window.calculateHireDriverFloorAndOffer();
    } else if (mode === 'hourly') {
...
window.setPreviewTiming = function(timing) {
    window.previewHireDriverState.timing = timing;
    const btnInstant = document.getElementById('preview-btn-timing-instant');
    const btnSchedule = document.getElementById('preview-btn-timing-schedule');
    const instantSec = document.getElementById('preview-timing-instant-details');
    const scheduleSec = document.getElementById('preview-timing-schedule-details');
...
// -------------------------------------------------------------------------
// Duration Pill Renderers
// -------------------------------------------------------------------------
window.renderPreviewHourlyPills = function() {
    const container = document.getElementById('preview-hourly-pills');
    if (!container) return;
    const options = [1, 2, 3, 4, 6, 8, 10, 12, 16, 20];
    const selected = window.previewHireDriverState.selectedHours || 4;
    const vehicleType = (window.selectedVehicle?.type || 'car').toLowerCase();
    const hourlyRate = parseFloat(window.redrivoSystemSettings?.[`${vehicleType}_hourly_rate`]) || 150;
    
    container.innerHTML = options.map(hrs => {
        const isSel = hrs === selected;
        const estRate = hrs * hourlyRate;
        return `
...
window.selectPreviewHour = function(hrs) {
...
window.renderPreviewDaywisePills = function() {
    const container = document.getElementById('preview-daywise-pills');
    if (!container) return;
    const options = [1, 2, 3, 4, 5, 7];
    const selected = window.previewHireDriverState.selectedDays || 1;
    const dailyRate = window.previewHireDriverState.tripType === 'round' ? 2100 : 1500;
    
    container.innerHTML = options.map(days => {
        const isSel = days === selected;
        const estRate = days * dailyRate;
        return `
...
window.selectPreviewDay = function(days) {
...
window.renderPreviewOutstationPills = function() {
    const container = document.getElementById('preview-outstation-pills');
    if (!container) return;
    const options = [
        { label: '12 Hrs', floor: 2160, offer: 2400 },
        { label: '16 Hrs', floor: 2560, offer: 2850 },
        { label: '20 Hrs', floor: 3000, offer: 3350 },
        { label: '1 Day', floor: 2100, offer: 2350 },
        { label: '2 Days', floor: 4200, offer: 4600 },
        { label: '3 Days', floor: 6300, offer: 6900 }
    ];
    const selected = window.previewHireDriverState.selectedOutstation || '1 Day';
...
window.selectPreviewOutstation = function(label, floor, offer) {
...
// Stepper Logic
// -------------------------------------------------------------------------
window.stepPreviewOffer = function(delta) {
    const floor = window.previewHireDriverState.floorAmount || 350;
    let current = (window.previewHireDriverState.offerAmount || 450) + delta;
    window.previewHireDriverState.userModifiedOffer = true;
    if (current < floor) {
        current = floor;
        if (typeof showToast === 'function') {
            showToast(`Minimum offer floor is ₹${floor.toLocaleString('en-IN')}`, 'info');
        }
    }
    window.previewHireDriverState.offerAmount = current;
    updatePreviewOfferDisplay();
...
window.updatePreviewOfferDisplay = function() {
    const disp = document.getElementById('preview-offer-amount-display');
    const floorLabel = document.getElementById('preview-offer-floor-label');
    const minusBtn = document.getElementById('preview-btn-offer-minus');
    
    const offer = window.previewHireDriverState.offerAmount || 450;
    const floor = window.previewHireDriverState.floorAmount || 350;
    
    if (disp) disp.textContent = offer.toLocaleString('en-IN');
    if (floorLabel) floorLabel.textContent = `Floor ₹${floor.toLocaleString('en-IN')}`;
...
window.addEventListener('hashchange', () => {
    if (window.location.hash === '#preview-hire-driver') {
        window.openHireDriverPreviewScreen();
    } else if (window.location.hash === '#preview-searching-driver') {
        window.openHireDriverSearchingPreview();
    } else if (window.location.hash === '#preview-driver-accepted') {
        window.openHireDriverAcceptedPreview();
    }
});

if (window.location.hash === '#preview-hire-driver') {
    window.addEventListener('DOMContentLoaded', () => window.openHireDriverPreviewScreen());
} else if (window.location.hash === '#preview-searching-driver') {
    window.addEventListener('DOMContentLoaded', () => window.openHireDriverSearchingPreview());
} else if (window.location.hash === '#preview-driver-accepted') {
    window.addEventListener('DOMContentLoaded', () => window.openHireDriverAcceptedPreview());
}